...
class PDAL {
    ...
    public function new() {
        var web = Server.web;
        var uri = 'mysql://${web.get("dbUser")}:${web.get("dbPassword")}@${web.get("dbHost")}:${web.get("dbPort")}/${web.get("dbName")}';
        _dal = Syntax.callNamedUntyped(DAL, {uri:uri, migrate:false, migrate_enabled:false, bigint_id:true});
